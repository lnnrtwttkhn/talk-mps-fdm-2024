...
## Data sharing with S3 Object Storage (min.io)
...
```{bash}
git annex initremote s3 type=S3 encryption=none autoenable=true host=minio.g.mpib-berlin.mpg.de port=443 protocol=https bucket=neurocode-neuro-data datacenter=EU
```
...
bucket=$BUCKET public=yes datacenter=mpib \
host=mpibberlin.com protocol=https port=443 autoenable=true
```


```{bash}
git annex initremote s3 type=S3 encryption=none \
bucket=neuro-data datacenter=EU \
host=mpibberlin.com protocol=https port=443 autoenable=true
```
